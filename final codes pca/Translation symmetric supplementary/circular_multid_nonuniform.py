# -*- coding: utf-8 -*-
"""
Created on Tue Jun 20 10:34:32 2023
...
def sampling_tc_around_locations_1d(locations,n_neurons_per_location,radii):
    tc_list=[(np.random.normal(loc=locations[i],scale=radii[i],
                                     size=n_neurons_per_location[i])%1)
                                     for i in range(len(locations))]
    tc_array=np.concatenate(tc_list)
    return tc_array

def gen_data_1d(fn, n_data, n_neurons,period,*args,tuning_centers='random'):
    # Get TC centers
...
def gen_data_multid(fn,n_data, n_neurons,n_dims,period,neurons_per_dimension,*args,tuning_centers='random'):
...
n_per_rl=8*np.ones(9)

#radius of each reward location
radii=0.05*np.ones(9)
...
n_neurons_per_dim_list=np.array([12])
...
for sigma in sigma_list:
    ...
    for n_idx,n_neurons in enumerate(total_neurons):
        ...
        for run_id in range(no_of_runs):
            start=time.time()
            #reward locations
            rl=np.random.uniform(0,1,size=(9,3))
            #generating multid data and performing pca
            #for d>4, this part becomes a lot of data in my laptop
            if d>1:
                data_d=gen_data_multid(resp_fn_d,n_data_pts,n_neurons,d,L,n_neurons_per_dim_uniform,
                            rl,n_per_rl,radii,tuning_centers='nonuniform')
            elif d==1:
                data_d=gen_data_1d(resp_fn_1d,n_data_pts_1d,n_neurons_per_dim_uniform,L,rl,n_per_rl,
...
fw=open('circular_multid_data/2023_06_19/linear_dim_data_nonuniform_tc_d={}'.format(d),'wb')
pickle.dump(to_save,fw)
fw.close()


plt.figure()
plt.scatter(mu_list_1,np.zeros(len(mu_list_1)),marker='o',label='uniformly distributed neurons')
plt.scatter(mu_list_2,np.zeros(len(mu_list_2)),marker='s',label='Added neurons')
plt.legend()
plt.savefig('plots/2023_06_21/Nonuniform neuron distribution d=1')
plt.show()
plt.close()


plt.figure()
plt.scatter(mu_list_1[:,0],mu_list_1[:,1],marker='o',label='uniformly distributed neurons')
plt.scatter(mu_list_2[:,0],mu_list_2[:,1],marker='s',label='Added neurons')
