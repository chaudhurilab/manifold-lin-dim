# -*- coding: utf-8 -*-
"""
Created on Tue Mar  1 12:54:57 2022
...
#put path for where you have saved data here
f1=os.path.abspath('..')
a=open(f1+'data/Multiplicative tuning/'+curr_date+'/product_sigmoid_response','wb')
sigmoid_response_data=pickle.load(a)
d_list=np.array([2,3,4,5,6,7,8])


#loading eigenvalues
#normalized eigenvals from d dimensional sim
eigenvals_d_median_frac=sigmoid_response_data['eigenvals_d_median_frac']
eigenvals_d_lower_frac=sigmoid_response_data['eigenvals_d_lower_frac']
eigenvals_d_upper_frac=sigmoid_response_data['eigenvals_d_upper_frac']

#normalized tensor product of 1d eigenvalues
eigenvals_tensor_d_median_frac=sigmoid_response_data['eigenvals_tensor_d_median_frac']
eigenvals_tensor_d_lower_frac=sigmoid_response_data['eigenvals_tensor_d_lower_frac']
eigenvals_tensor_d_upper_frac=sigmoid_response_data['eigenvals_tensor_d_upper_frac']

#loading linear dimension
linear_dims_tensor_prod_lower=sigmoid_response_data['linear_dims_tensor_prod_lower']
linear_dims_tensor_prod_median=sigmoid_response_data['linear_dims_tensor_prod_median']
linear_dims_tensor_prod_upper=sigmoid_response_data['linear_dims_tensor_prod_upper']
...
#1d eigenvalues
eigenvals_1d_median=sigmoid_response_data['eigenvals_1d']
...
#plt.savefig(f1+'plots/'+curr_date+'/linear_dim_vs_true_dim_prod_sigmoid_response_cutoff_09.pdf',transparent=True)
#plt.legend()
plt.show()
plt.close()
