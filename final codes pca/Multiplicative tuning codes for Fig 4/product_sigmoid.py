# -*- coding: utf-8 -*-
"""
Created on Tue Jan  4 19:56:21 2022

@author: Anandita
"""
''' Generate data from sigmoidal tuning curves with varying slopes along each dimension. 
    The sigmoids along each dimension has the same set of sloped so that the
    ...
    corresponding to sigmoidal tuning curves with varying slopes in Figure 4 of the
    paper.'''
...
# The latent variables x, lie in a region [0,period]  or[0,period]^d
period=2
...
for d_idx,d in enumerate(d_list):
    ...
    params_list_1d=[[params] for params in zip(mu_list_1d,slopes)]
    ...
    for run_idx in range(no_of_runs):
         ...
         #calculated from d dimensional data
         if d<=4:
         ...
         print('Run:',run_idx,'d=',d,' Time required:',time.time()-start)
...
#finding the median, 10th and 90th percentiles of the eigenvalues and the 
#linear dimension from both d dimensional sim and tensor product of eigenvalues

#1d eigenvalues and fraction of 1d eigenvalues
eigenvals_1d_median=np.median(eigenvalues_matrix_1d,axis=0)
eigenvals_1d_median_frac=eigenvals_1d_median/np.sum(eigenvals_1d_median)

eigenvals_1d_lower=np.percentile(eigenvalues_matrix_1d,10,axis=0)
eigenvals_1d_lower_frac=eigenvals_1d_lower/np.sum(eigenvals_1d_lower)

eigenvals_1d_upper=np.percentile(eigenvalues_matrix_1d,90,axis=0)
eigenvals_1d_upper_frac=eigenvals_1d_upper/np.sum(eigenvals_1d_upper)
